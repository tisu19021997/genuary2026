...
class Tile {
  ...
  constructor(q, r, size) {
    ...
    this.x = colSpacing * q;
    this.y = rowSpacing * r + (q % 2) * oddColOffset;
  ...
  draw() {
    ...
    for (let i = 0; i < 6; i++) {
      // Angle degree = 60 * i
      // Angle radian = PI / 180 * (60 * i) = i * PI / 3
      const angle = (i * PI) / 3;
  ...
  contains(px, py) {
    ...
    const r = ((-1 / 3) * dx + (sqrt(3) / 3) * dy) / this.size;
...
class Agent {
  ...
  draw() {
    ...
    fill(255, 100, 100);
    noStroke();
    circle(this.tile.x, this.tile.y, this.grid.tileSize * 0.5);
    pop();
  }
...
class Grid {
  ...
  evolveAll() {
    ...
    for (const tile of toEvolve) {
      tile.level = EVOLUTION[tile.level].next;
      evolved++;
    }
  ...
  decayPass() {
    ...
    for (const tile of toDemote) {
      tile.level--;
      if (tile.level < LEVEL.HOUSE) {
        tile.state = STATE_SPACE.EMPTY;
        tile.level = LEVEL.EMPTY;
      }
    }
...
function updateHoveredTile() {
  const offset = grid.tileSize;
  const localX = mouseX - offset;
  const localY = mouseY - offset;
...
function draw() {
  ...
  if (hoveredTile && hoveredTile.state === STATE_SPACE.CRYSTALLIZED) {
    hoveredTile.drawServiceRange(grid);
  }
